...
jQuery.extend(Boxy, {
    ...
    industry: function (arrind ,value, callback, options) {
        ...
        //脚部
        //var foot = jQuery("<div id='ind-foot'><span id='ind-msg'>注：最多只能选择5个类别</span><span id='ind-cancel' >取消</span><span id='ind-submit'>确定</span></div>");
		var foot = jQuery("<div id='ind-foot'><span id='ind-cancel' >取消</span><span id='ind-submit'>确定</span></div>");
        //将头部，主体内容和脚部加入
                ...
                if (items != "") {
                    ...
                    jQuery(".indResultItem", element).click(function (event) {
                        ...
                        if (ele.attr("class") && ele.attr("class") == "indResultItem") {
                            DelIndSelItem(this);//DelIndSelItem(event);
                            UpdateIndSelItems();
                            UpdateIndCheckedStatus();
                        }
                    });
                    ...
                    resultEle.hover(function (event) {
                        ...
                        jQuery("div", targetEle).css("background-image", "url(../image/boxy/del_hover_left.gif)");
                        jQuery("span", targetEle).css("background-image", "url(../image/boxy/del_hover_center.gif)");
                    }, function (event) {
                        //var targetEle = jQuery(event.currentTarget);
                        var targetEle = jQuery(this);
                        targetEle.css("background-image", "url(../image/boxy/del_normal_right.gif)");
                        jQuery("div", targetEle).css("background-image", "url(../image/boxy/del_normal_left.gif)");
                        jQuery("span", targetEle).css("background-image", "url(../image/boxy/del_normal_center.gif)");
                    });
                }
            }
        };
        ...
        function DelIndSelItem(event) {
			...
			var element = jQuery(event);//var element = jQuery(event.currentTarget);
        ...
        jQuery("a", body).click(function (event) {
        	//event.currentTarget : handson updated this to this;
            var id = jQuery(this).attr("id").split("Item")[1];
        ...
        jQuery("#ind-submit", foot).click(function () {
            ...
            var clicked = this;
            Boxy.get(this).hide(function () {
                if (callback) callback(value,arrSelItems);
            });
        });
    ...
    area: function (values, showns, callback, options) {
                ...
                if (items != "") {
                    ...
                    jQuery(".jobResultItem", element).click(function (event) {
                        ...
                        if (ele.attr("class") && ele.attr("class") == "jobResultItem") {
                            DelAreaSelItem(this);//DelAreaSelItem(event);
                            UpdateAreaSelectorSelItems();
                            UpdateAreaSelectorCheckedStatus();
                        }
                    });
                    ...
                    var resultEle = jQuery("li", element);
                    resultEle.hover(function (event) {
                         var targetEle = jQuery(this);//var element = jQuery(event.currentTarget);
                        targetEle.css("background-image", "url(../image/boxy/del_hover_right.gif)");
                        jQuery("div", targetEle).css("background-image", "url(../image/boxy/del_hover_left.gif)");
                        jQuery("span", targetEle).css("background-image", "url(../image/boxy/del_hover_center.gif)");
                    }, function (event) {
        ...
        function DelAreaSelItem(event) {
            ...
            var ele = jQuery(event);//var ele = jQuery(event.currentTarget);
    ...
    job: function (value, shown, callback, options) {
            ...
            if (arrJobSelectedParents.length + arrJobSelectedChildren.length == 0) {
                ...
                if (items != "") {
                    ...
                    jQuery(".jobResultItem", element).click(function (event) {
                        ...
                        if (ele.attr("class") && ele.attr("class") == "jobResultItem") {
                            DelJobSelItem(this);//DelJobSelItem(event);
                            UpdateJobSelItems();
        ...
        function SetChildItems() {
            ...
            if ((onShownParent != "") && (childrenLayer.attr("id") != "parId" + onShownParent)) {
                ...
                jQuery("a", childrenLayer).click(function (event) {
                    childrenClickListener(this);//childrenClickListener(event);
                });
            }
        };
        ...
        function childrenClickListener(event) {
            ...
            //获取父项ID
            var parentId = jQuery(event).parent().attr("id").split("Id")[1];//var parentId = jQuery(event.currentTarget).parent().attr("id").split("Id")[1];
            ...
            if (!isParContained) {
                var id = jQuery(event).attr("id").split("Item")[1];//var id = jQuery(event.currentTarget).attr("id").split("Item")[1];
                var isContain = false;
                var children = new Array();
                //更新选择结果集合，存在则移除，否则添加
                for (var i = 0; i < arrJobSelectedChildren.length; i++) {
    ...
    industryChild: function (arrclass,arrjob,value, shown, callback, options) {
        options = jQuery.extend({ modal: true, closeable: true, fixed: false },
                                options || {},
                                { show: true, unloadOnHide: true });
       ...
       // var foot = jQuery("<div id='job-foot'><span id='job-msg'>注：最多只能选择20个类别</span><span id='job-cancel' >取消</span><span id='job-submit'>确定</span></div>");

		var foot = jQuery("<div id='job-foot'><span id='job-cancel' >取消</span><span id='job-submit'>确定</span></div>");
        ...
        function ParentOption(id, name) {
			//<input type='checkbox' />
            var value = "<a id='parItem" + id + "' href='javascript:void(0);' class='parentItem'><span id='parTd" + id + "' style='padding-left:2px;'>" + name + "</span></a>";
            return value;
        };
        ...
        function childrenClickListener(event) {
            ...
            if (!isParContained) {
                var id = jQuery(event).attr("id").split("Item")[1];
                var isContain = false;
                var children = new Array();
                //更新选择结果集合，存在则移除，否则添加
                for (var i = 0; i < arrJobSelectedChildren.length; i++) {
        ...
        jQuery("#job-submit", foot).click(function () {
            ...
            var clicked = this;
            Boxy.get(this).hide(function () {
                if (callback) callback(value,arrJobSelectedChildren);
            });
        });

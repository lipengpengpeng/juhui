...
<html xmlns="http://www.w3.org/1999/xhtml">
				 ...
				 $("#gjfAttrValueForm").validate({
					...
					messages:{  
						"setDivi.consumptionMin" : {
							 ...
							 /* number:"累计消费最小金额只能输入数字" */
						 },
						 "setDivi.consumptionMax" : {
							 required:"请输入累计消费最大金额",
							/*  number:"累计消费最大金额只能输入数字", */
						 },
						 "setDivi.consumption" : {
							 required:"请输入分红权个数比例金额",
							 /* number:"分红权个数比例金额只能输入数字", */
						 },						
					},
	...
	<body>
		<div class="rhead">
			<div class="rpos">当前位置: 分红权个数设置- 编辑分红权个数信息</div>
		...
		<form id="gjfAttrValueForm" action="${ctx}/subsystem/tradeInfoAction!newSetDivi.action" method="post" name="gjfAttrValueForm" >
			<table  align="center" class="listTable3">
				<tr>
					...
					<td><input type="text"  name="setDivi.consumptionMin" value="${resultVo.result.consumptionMin}"/></td>
				</tr>
				<tr>
					<td class="pn-flabel" width="100px">累计消费最大金额</td>
					<td><input type="text"  name="setDivi.consumptionMax" value="${resultVo.result.consumptionMax}"/></td>
				</tr>
				<tr>
					<td class="pn-flabel" width="100px">分红权个数比例金额</td>
					<td><input type="text"  name="setDivi.consumption" value="${resultVo.result.consumption}"/></td>
				</tr>
				
				<input type="hidden"  name="setDivi.status" value="${resultVo.result.status}"/>
				<input type="hidden"  name="setDivi.id" value="${resultVo.result.id}"/>
				<tr>
					<td></td>
					<td>

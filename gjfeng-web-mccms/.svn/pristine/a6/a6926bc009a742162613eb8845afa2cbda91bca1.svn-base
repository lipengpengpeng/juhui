	...
	<body>
		...
		<form action="countInfoAction!retrieveMemberByCondition.action" method="post">
			<table class="listTable2" >
				<tr>
					<td>
						会员名称：<input type="text" placeholder="请输入会员名称" value="${name }" name="name" id="name"/>&nbsp;&nbsp;
						会员昵称：<input type="text" placeholder="请输入会员名称" value="${nickName }"  name="nickName" id="nickName"/>&nbsp;&nbsp;
		...
		<table class="listTable3" onmouseover="changeto()" onmouseout="changeback()">
			...
			<c:forEach var="bean" items="${memberInfos}" varStatus="status">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${not empty bean.lowLevelCumulativeMoney }">${bean.lowLevelCumulativeMoney }</c:when>
							<c:when test="${empty bean.lowLevelCumulativeMoney}">0.00</c:when>
						</c:choose>
						
					</td>
					<td>
						<c:choose>
							<c:when test="${not empty bean.lowLevelCumulativeMoney }">
								<a href="countInfoAction!findMemberLowLevelConsume.action?id=${bean.id }">查看</a>
							</c:when>
						</c:choose>
					</td>
				</tr>

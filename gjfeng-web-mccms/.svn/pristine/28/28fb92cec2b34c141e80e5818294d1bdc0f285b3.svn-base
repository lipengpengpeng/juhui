...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="rhead">
			...
			<div class="ropt">
				<input type="button" value='<s:text name="common.word.new" />' onclick="location.href='attrValueAction!newAttrValue.action'"/>
			</div>
			<div class="clear"></div>
		</div>
		...
		<table class="listTable3" onmouseover="changeto()" onmouseout="changeback()">
			...
			<c:forEach var="bean" items="${gjfAttrValues}"
				varStatus="status">
				<tr>
					<td>${status.count}</td>
					<td>${bean.attrId.attrName}</td>
					<td>${bean.columnId }</td>
					...
					<td>
						<a href="attrValueAction!retrieveAttrValueById.action?id=${bean.id}">编辑</a>
						&nbsp; &nbsp;
						<c:if test="${bean.status eq 0}">
							<%-- <a href="javascript:void(0);" onclick="ajaxUpdateStatus(${bean.id},1,this,${ctx}/ajaxAttrValueAction!modifyAttrValueStatus)">启用</a> --%>
								<a href="javascript:void(0);" onclick="updateStatus(${bean.id},1,this)">启用</a>
								<input type="hidden" class="security-token" value="${bean.token}"/>
						</c:if>
						<c:if test="${bean.status eq 1}">
							<%-- <a href="javascript:void(0);" onclick="ajaxUpdateStatus(${bean.id},0,this,${ctx}/ajaxAttrValueAction!modifyAttrValueStatus)">停用</a> --%>
		...
		<c:if test="${not empty gjfAttrValues}">
			<s:form action="attrValueAction!retrieveAttrValueByPage.action"

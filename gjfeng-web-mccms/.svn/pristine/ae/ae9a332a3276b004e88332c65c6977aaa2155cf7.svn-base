 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	customHotkey:			""			// EITHER a charCode OR a character - MUST be set 'by pane'
 ...
 */
	...
	var setTimer = function (pane, action, fn, ms) {
		...
		,	name = "layout_"+ o.id +"_"+ pane +"_"+ action // UNIQUE NAME for every layout-pane-action
		;
 ...
 */
	...
	var create = function () {
		...
		$.each(c.borderPanes.split(","), function (i,pane) {
			var s = o[pane];
			if (s.enableCursorHotkey || s.customHotkey) {
	...
	var initOptions = function () {
		...
		$.each("paneSelector,resizerCursor,customHotkey,name".split(","),
		...
		// generate a random 'ID#' to identify this layout - used to create global namespace for timers
		o.id = Math.floor(Math.random() * 10000);
	};
	...
	var initPanes = function () {
		...
		$.each(c.allPanes.split(","), function() {
			...
			s.initSize = s.size; // save size option for reference
	...
	var initHandles = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			if (s.closable) {
				...
				if (s.togglerContent_open) // ui-layout-open
					$T.append("<span class='content content_open'>"+ s.togglerContent_open +"</span>");
				if (s.togglerContent_closed) // ui-layout-closed
					$T.append("<span class='content content_closed'>"+ s.togglerContent_closed +"</span>");
 ...
 */
	...
	var close = function (pane, force) {
		...
		function close_2 () {
			...
			execUserCallback(pane, s.onclose); // see if there is a callback for this pane
	...
	var open = function (pane, slide) {
		...
		var maxSize = getPaneMinMaxSizes(pane).maxSize;
		if (s.size > maxSize) // pane is too big! resize it before opening
			$P.css( c[pane].sizeType, max(1, cssSize(pane, maxSize)) );
		...
		function open_2 () {
			...
			// see if there is a callback for this pane
			execUserCallback(pane, s.onopen);
	...
	var sizePane = function (pane, size) {
		...
		if (s.maxSize > 0) size = min(s.maxSize, size);
		if (s.minSize > 0) size = max(s.minSize, size);
		...
		execUserCallback(pane, s.onresize);
	};
	...
	var sizeHandles = function (panes, onInit) {
		...
		$.each(panes.split(","), function() {
			...
			if ($T) {
				...
				var $TC = (s["togglerContent"+state] ? $T.children(".content_"+(s.isClosed ? "closed" : "open")) : false);
				...
				if (dir == "horz") { // north/south
					...
					if ($TC) { // CENTER the toggler content SPAN
						offset = Math.floor((width-$TC.outerWidth())/2);
						$TC.css("marginLeft", (offset > 0 ? offset : 0));
					}
				}
				else { // east/west
					...
					if ($TC) { // CENTER the toggler content SPAN
						offset = Math.floor((height-$TC.outerHeight())/2);
						$TC.css("marginTop", (offset > 0 ? offset : 0));
					}
				}
	...
	var resizeAll = function () {
		...
		,	d = cDims = getContainerDims() // IMPORTANT - update container size var
		,	checkH	= (d.innerHeight < oldH)
		,	checkW	= (d.innerWidth < oldW)
		,	s, dir, maxSize
		;
		...
		if (checkH || checkW)
			...
			$.each(["south","north","east","west"], function(i,pane) {
				...
				if (!s.isClosed && ((checkH && dir=="horz") || (checkW && dir=="vert"))) {
					maxSize = getPaneMinMaxSizes(pane).maxSize;
					// shrink pane if 'too big' to fit
					if (s.size > maxSize)
						sizePane(pane, maxSize);
				}
			});
	...
	function keyDown (evt) {
		...
		if (!CTRL && !SHIFT)
			...
			$.each(c.borderPanes.split(","), function(i,p) { // loop each pane to check its hotkey
				s = o[p];
				k = s.customHotkey;
				m = s.customHotkeyModifier; // if missing or invalid, treated as "CTRL+SHIFT"
		...
		// validate pane
		s = o[pane]; // get pane options
		if (!s.enableCursorHotkey || s.isHidden || !$Ps[pane]) return true;

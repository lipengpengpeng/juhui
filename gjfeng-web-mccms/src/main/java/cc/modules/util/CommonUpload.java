...
public class CommonUpload {
	...
	public boolean uploadFile() throws Exception {
		...
		if (uploadFileName != null && !uploadFileName.equals("")) {
			try {
				...
				newFileName = (new StringBuffer(String.valueOf((new DateHelper()).getRandomNum()))).append(".").append(inputFileExt)
					.toString();
				String outputFilePath = (new StringBuffer(String.valueOf(getSavePath()))).append(Constants.FILE_SEPARATOR)
	...
	public boolean delectFile() throws Exception {
		File delFile = new File((new StringBuffer(String.valueOf(getSavePath()))).append(Constants.FILE_SEPARATOR)
			.append(oldUploadFileName).toString());
		if (delFile.exists()) {
			try {
				delFile.delete();
			} catch (Exception e) {
		 ...
		 * 删除压缩文件
		 */
		File delZipFile = new File((new StringBuffer(String.valueOf(getZipSavePath()))).append(Constants.FILE_SEPARATOR)
			.append("zip_" + oldUploadFileName).toString());

...
public class EpColumnManagerDaoImpl extends BaseManagerDaoImpl implements EpColumnManagerDao {
	...
	public void saveEnterpriseColumn(EnterpriseColumn enterpriseColumn) {
		 ...
		 */
		if (enterpriseColumn.getTypeColumn().getTemplateType() == null
			|| "".equals(enterpriseColumn.getTypeColumn().getTemplateType())) {
	...
	public void updateEnterpriseColumn(EnterpriseColumn enterpriseColumn, Authorities auth) {
		...
		String linkUrl0=enterpriseColumn.getLinkUrl();
		String linkUrl = buildLinkUrl(enterpriseColumn);
		if (linkUrl != null) {
			enterpriseColumn.setLinkUrl(linkUrl);
		}
		enterpriseColumn.setLinkUrl(linkUrl0);
		// 更新栏目信息
	...
	public List<IndexInfoBean> findFrontInfoFrontNumNotNull() {
		...
		if (tempList != null) {
			...
			while (iterator.hasNext()) {
				...
				if ("link".equals(ec.getTypeColumn().getTemplateType())) {
				...
				} else if ("list".equals(ec.getTypeColumn().getTemplateType())
					|| "download".equals(ec.getTypeColumn().getTemplateType())
					...
					/*
					 * SQL.append(" and (enterpriseColumn.id = "
					 * ).append(ec.getId());
					 * 
					 * Iterator iter =
					 * temp.getEnterpriseColumnList().iterator();
					 * 
					 * while (iter.hasNext()) { EnterpriseColumn e =
					 * (EnterpriseColumn) iter.next(); SQL.append(
					 * " or enterpriseColumn.id = ").append(e.getId()); }
					 * SQL.append(")");
					 * 
					 * temp.setMcProductInfoList(this.mcProductInfoDao.
					 * findMcProductInfoByWhere(SQL.toString()));
					 */
					temp.setMcProductInfoList(this.productColumnDao.findProductByColumnPro(null, ec));
				}
	...
	public List getColumnByIds(Long[] ids) {
		return this.epColumnDao.findColumnByIds(ids);
	}
	...
	public List findColumnByFatherId(Long id) {
		return this.epColumnDao.findColumnByFatherId(id);
	}

	@Override
	public List<EnterpriseColumn> findProductColumn() {
		return epColumnDao.findProductColumn();
	}
	...
	public EnterpriseColumn findFatherByid(Long id) {
		return this.epColumnDao.findFatherId(id);
	}

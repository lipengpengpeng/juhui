...
public class FileHandler {
	...
	public static boolean createFile(final String inputStr, final String targetFilePath) {
		try {
			...
			//LogUtil.logException("file write exception", e);
			return false;
		}
	}
	...
	public static void writeBase64File(final String base64FileContent, final String outFilePath) throws IOException {
		// Base64解码
		byte[] b = Base64.decodeBase64(base64FileContent);
	 ...
	 */
	private static void createFolderIfNotExists(String filePath) throws FileExistsException {
		
		String folderPath = filePath.substring(0, filePath.lastIndexOf(File.separator));
		File target = new File(folderPath);
		System.out.println("文件夹："+filePath+"\tstatus:"+target.exists());
		if (!target.exists()) {
			if (!target.mkdirs()) {
				throw new FileExistsException("创建文件异常：" + target.getAbsolutePath());
			}
		} else {

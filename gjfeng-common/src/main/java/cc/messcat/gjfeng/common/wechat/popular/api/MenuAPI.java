...
public class MenuAPI extends BaseAPI{
	...
	private static MenuButtons getMenu() throws UnsupportedEncodingException {
		...
		childBtn3.setName("新手指南");
		childBtn3.setType("view");
		String url2 = URLEncoder.encode(CommonProperties.CLIENT_PROJECT_URL+"/navigation.htm?columnId=28&columnType=content", "UTF-8");
		childBtn3.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WeChatPublicNumberConfig.GJFENG_APP_ID()+"&redirect_uri="+url2+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		
		
		MenuButtons.Button childBtn4 = new MenuButtons.Button();
		childBtn4.setName("生成二维码");
		childBtn4.setType("view");
		String url3 = URLEncoder.encode(CommonProperties.CLIENT_PROJECT_URL+"/epMemberCenterAction!generateQrcode.action", "UTF-8");
		childBtn4.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WeChatPublicNumberConfig.GJFENG_APP_ID()+"&redirect_uri="+url3+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		
		

		MenuButtons.Button childBtn5 = new MenuButtons.Button();
		childBtn5.setName("会员中心");
		childBtn5.setType("view");
		String url4 = URLEncoder.encode(CommonProperties.CLIENT_PROJECT_URL+"/epMemberCenterAction.action", "UTF-8");
		childBtn5.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WeChatPublicNumberConfig.GJFENG_APP_ID()+"&redirect_uri="+url4+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		
		
		MenuButtons.Button childBtn6 = new MenuButtons.Button();
		childBtn6.setName("模式解说");
		childBtn6.setType("view");
		String url5 = URLEncoder.encode(CommonProperties.CLIENT_PROJECT_URL+"/navigation.htm?columnId=29&columnType=content", "UTF-8");
		childBtn6.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WeChatPublicNumberConfig.GJFENG_APP_ID()+"&redirect_uri="+url5+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		
		MenuButtons.Button childBtn12 = new MenuButtons.Button();
		childBtn12.setName("我的分享");
		childBtn12.setType("view");
		String url11 = URLEncoder.encode(CommonProperties.CLIENT_PROJECT_URL+"/epNewsListMoreAction!newsList.action", "UTF-8");
		childBtn12.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WeChatPublicNumberConfig.GJFENG_APP_ID()+"&redirect_uri="+url11+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		*/

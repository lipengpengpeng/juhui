...
public class ObjValid {
	...
	public static boolean isNotValid(Object... objs) {
		boolean flag=isValid(objs);
		if (flag) {
			return false;
		}
		return true;
	 ...
	 * @param obj
	 */
	public static boolean isValidMap(Map... attrs) {
		if(null != attrs && !"[]".equals(attrs[0].values().toString()) && attrs.length > 0 && !"".equals(attrs))
			return true;
		return false;
	}
	...
	public static Map<String,Object> objectToMap(Map<String, Object> map, Object object, boolean isField, String childfieldName){
		if (null!=object) {
			...
			for(Field f : object.getClass().getDeclaredFields()){
				...
				//将属性名称和值存入map中；
				map.put(fieldName.toString(), o==null?"":o.toString());
				//清零，重新构建下一个变量的字符串
				fieldName.setLength(0);          
			}
		}

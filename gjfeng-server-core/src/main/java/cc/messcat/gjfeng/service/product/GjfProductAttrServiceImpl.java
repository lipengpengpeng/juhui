...
public class GjfProductAttrServiceImpl implements GjfProductAttrService {
	...
	public ResultVo findProAttrByProIdInApp(Long proId) {
		...
		Map<String, Object> dataMap = new LinkedHashMap<String, Object>();
		// 1.查询出该商品所有属性，排除掉重复的
		Map<String, Object> pattrs=new HashMap<>();
		pattrs.put("id", proId);
		GjfProductInfo productInfo = gjfProductAttrDao.query(GjfProductInfo.class, pattrs);
		if(!BeanUtil.isValid(productInfo)){
			throw new MyException(400, "商品不存在", null);
		}
		List<GjfProductAttr> gjfProductAttrs=null;
		if(!"5".equals(productInfo.getSuorceGoods())){
			gjfProductAttrs = gjfProductAttrDao.findProAttrByProId(proId);
		}
		if (ObjValid.isValid(gjfProductAttrs)) {
			...
			dataMap.put("content", gjfProductAttrs.get(0).getProductId().getContent());
			dataMap.put("pointNeedMoney",  gjfProductAttrs.get(0).getProductId().getPointNicePrice());
			dataMap.put("purchasNum", gjfProductAttrs.get(0).getProductId().getPurchasNum());
			dataMap.put("isCanUserCou", gjfProductAttrs.get(0).getProductId().getIsCanUserCou());
			dataMap.put("goodSoure", gjfProductAttrs.get(0).getProductId().getSuorceGoods());
			dataMap.put("isWholesale", gjfProductAttrs.get(0).getProductId().getIsWholesale());
			// 2.根据商品属性查询该商品属性下的属性值
			...
			stockMap.put("standardTotalAmount", gjfProductAttrStock.getProductId().getStandardPrice());
			stockMap.put("honourTotalAmount", gjfProductAttrStock.getProductId().getHonourPrice());
			stockMap.put("honourPreferentialMoney", gjfProductAttrStock.getStandardPrice().subtract(gjfProductAttrStock.getHonourPrice()));
			stockMap.put("standardPreferentialMoney", gjfProductAttrStock.getPrice().subtract(gjfProductAttrStock.getStandardPrice()));
			dataMap.put("productAttrStock", stockMap);
		}else{
			
			dataMap.put("proName", productInfo.getName());
			dataMap.put("proId", productInfo.getId());
			dataMap.put("proImg", productInfo.getImgUrl());
			dataMap.put("para1", productInfo.getPara1());
			dataMap.put("para2",productInfo.getPara2());
			dataMap.put("para3", productInfo.getPara3());
			dataMap.put("para4", productInfo.getPara4());
			dataMap.put("para5", productInfo.getPara5());
			dataMap.put("content", productInfo.getContent());
			dataMap.put("pointNeedMoney",  productInfo.getPointNicePrice());
			dataMap.put("purchasNum", productInfo.getPurchasNum());
			dataMap.put("benerfitMoney", productInfo.getBenerfitMoney());
			dataMap.put("isCanUserCou", productInfo.getIsCanUserCou());
			dataMap.put("goodSoure", productInfo.getSuorceGoods());
			dataMap.put("isWholesale",productInfo.getIsWholesale());
			dataMap.put("multipleNumber",productInfo.getMultipleNumber());
			// 2.根据商品属性查询该商品属性下的属性值			
			dataMap.put("attrs", null);
			
			// 查询第一个属性值的库存和价格
			Map stockMap = new HashMap<>();
			stockMap.put("id", null);
			stockMap.put("productAttrIds", null);
			stockMap.put("repertory",productInfo.getRepertory());
			stockMap.put("price",productInfo.getPrice());
			stockMap.put("standardTotalAmount", productInfo.getStandardPrice());
			stockMap.put("honourTotalAmount", productInfo.getHonourPrice());
			stockMap.put("honourPreferentialMoney", productInfo.getStandardPrice().subtract(productInfo.getHonourPrice()));
			stockMap.put("standardPreferentialMoney", productInfo.getPrice().subtract(productInfo.getStandardPrice()));
			dataMap.put("productAttrStock", stockMap);
		}
		return new ResultVo(200, "查询成功", dataMap);
	}

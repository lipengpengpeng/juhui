...
public class GjfProductInfoDaoImpl extends BaseHibernateDaoImpl<Serializable> implements GjfProductInfoDao {
	...
	public List<ProductInfoVo> findProByFatherColumnId(Long columnId, int pageNo, int pageSize, String shopType, String orderType,
		...
		if (shopType.equals("1")) {
			...
			parMap.put("columnId", columnId);
			sql.append("SELECT p.ID_ AS id,p.NAME_ AS name,p.PRICE_ AS price,p.IMG_URL_ AS imgUrl,p.MULTIPLE_NUMBER_ as multipleNumber");
			sql.append(
			...
			return getCurrentSession().createSQLQuery(sql.toString()).addScalar("id", LongType.INSTANCE)
				.addScalar("name", StringType.INSTANCE).addScalar("price", BigDecimalType.INSTANCE)
				.addScalar("imgUrl", StringType.INSTANCE).addScalar("multipleNumber", StringType.INSTANCE).setResultTransformer(Transformers.aliasToBean(ProductInfoVo.class))
				.setProperties(parMap).setFirstResult((pageNo - 1) * pageSize).setMaxResults(pageSize).list();
		}
	}
	...
	public List findAllCartMember(Long memberId) {
		...
		String[] param={"id","memberId","goodsId","goodsAttrStockId","goodsAttr","goodsNum","addTime","logist"};
	...
	public List<ProductInfoVo> findMerchantProcurementProduct(Long columnId,String likeName,Integer pageNo,Integer pageSize,String discount) {
		...
		if(BeanUtil.isValid(columnId)){
			parMap.put("columnId", columnId);
			sql.append("SELECT p.ID_ AS id,p.NAME_ AS name,p.PRICE_ AS price,p.IMG_URL_ AS imgUrl,p.IS_CAN_USER_COU_ as isCanUserCou,p.POINT_NICE_PRICE_ as pointNicePrice,p.STANDARD_PRICE_ as standardPrice,p.HONOUR_PRICE_ as honourPrice,p.MULTIPLE_NUMBER_ as multipleNumber,p.REPERTORY_ as repertory,p.SALES_NUM_ as salesNum");
			sql.append(
				" FROM gjf_product_column pc LEFT JOIN gjf_product_info p ON pc.PRODUCT_ID_ = p.ID_ LEFT JOIN gjf_enterprise_column c ON pc.COLUMN_ID_ = c.ID_  WHERE c.ID_=:columnId AND p.ADUIT_STATUS_='1' AND p.STATUS_='1' AND p.IS_WHOLESALE_='1' ");
			
			if (StringUtil.isNotBlank(likeName)) {
				parMap.put("likeValue", "%" + likeName.replace("%", "\\%") + "%");
				sql.append(
					" AND (p.NAME_ LIKE :likeValue OR p.KEYWORDS_ LIKE :likeValue OR p.DESCRIPTION_ LIKE :likeValue OR s.STORE_NAME_ LIKE :likeValue)");
			}
			if(BeanUtil.isValid(discount)){				
				if("1".equals(discount)){//一折
					sql.append(" AND (p.GCOST_PRICE_*10/p.PRICE_)<=1");
				}else if("2".equals(discount)){
					sql.append(" AND (p.GCOST_PRICE_*10/p.PRICE_)>1 and (p.GCOST_PRICE_*10/p.PRICE_)<=2");
				}else if("3".equals(discount)){
					sql.append(" AND (p.GCOST_PRICE_*10/p.PRICE_)>3 and (p.GCOST_PRICE_*10/p.PRICE_)<=4");
				}else if("4".equals(discount)){
					sql.append(" AND (p.GCOST_PRICE_*10/p.PRICE_)>4");
				}
			}
			sql.append(" ORDER BY p.SALES_NUM_ DESC");
			return getCurrentSession().createSQLQuery(sql.toString()).addScalar("id", LongType.INSTANCE)
					.addScalar("name", StringType.INSTANCE).addScalar("price", BigDecimalType.INSTANCE)
					.addScalar("imgUrl", StringType.INSTANCE).addScalar("isCanUserCou", StringType.INSTANCE).addScalar("pointNicePrice", BigDecimalType.INSTANCE).addScalar("standardPrice", BigDecimalType.INSTANCE).addScalar("honourPrice", BigDecimalType.INSTANCE)
					.addScalar("multipleNumber", StringType.INSTANCE).addScalar("repertory", LongType.INSTANCE).addScalar("salesNum", LongType.INSTANCE).setResultTransformer(Transformers.aliasToBean(ProductInfoVo.class))
					.setProperties(parMap).setFirstResult((pageNo - 1) * pageSize).setMaxResults(pageSize).list();
		}else{
			sql.append("SELECT p.ID_ AS id,p.NAME_ AS name,p.PRICE_ AS price,p.IMG_URL_ AS imgUrl,p.IS_CAN_USER_COU_ as isCanUserCou,p.POINT_NICE_PRICE_ as pointNicePrice,p.STANDARD_PRICE_ as standardPrice,p.HONOUR_PRICE_ as honourPrice,p.MULTIPLE_NUMBER_ as multipleNumber,p.REPERTORY_ as repertory,p.SALES_NUM_ as salesNum");
			sql.append(
				" FROM  gjf_product_info p  WHERE  p.ADUIT_STATUS_='1' AND p.STATUS_='1' and p.IS_WHOLESALE_='1'");

			if (StringUtil.isNotBlank(likeName)) {
				parMap.put("likeValue", "%" + likeName.replace("%", "\\%") + "%");
				sql.append(
					" AND (p.NAME_ LIKE :likeValue OR p.KEYWORDS_ LIKE :likeValue OR p.DESCRIPTION_ LIKE :likeValue )");
			}
			if(BeanUtil.isValid(discount)){				
				if("1".equals(discount)){//一折
					sql.append(" AND (p.STANDARD_PRICE_/p.PRICE_)<=1");
				}else if("2".equals(discount)){
					sql.append(" AND (p.STANDARD_PRICE_/p.PRICE_)>1 and (p.STANDARD_PRICE_/p.PRICE_)<=2");
				}else if("3".equals(discount)){
					sql.append(" AND (p.STANDARD_PRICE_/p.PRICE_)>3 and (p.STANDARD_PRICE_/p.PRICE_)<=4");
				}else if("4".equals(discount)){
					sql.append(" AND (p.STANDARD_PRICE_/p.PRICE_)>4");
				}
	...
	public List<GjfEnterpriseColumn> findIndexPro(String goodsType, Long pageTypeId) {
		...
		String hql = "select c from GjfEnterpriseColumn c where c.state='1' and c.typeColumn.id=? and c.isValidInNav='0' and c.intro='index' and c.father in(";
	...
	@SuppressWarnings("unchecked")
	@Override
	public Pager findMerchantProcurementProductInPc(Long columnId, String likeName, Integer pageNo, Integer pageSize,
			String discount) {
		StringBuffer sql=new StringBuffer();
		Map<String, Object> parMap=new HashMap<>();
		...
		if(BeanUtil.isValid(columnId)){
			...
			sql.append(" ORDER BY p.SALES_NUM_ DESC");
		    List<ProductInfoVo> resultList=getCurrentSession().createSQLQuery(sql.toString()).addScalar("id", LongType.INSTANCE)
					.addScalar("name", StringType.INSTANCE).addScalar("price", BigDecimalType.INSTANCE)
					.addScalar("imgUrl", StringType.INSTANCE).addScalar("isCanUserCou", StringType.INSTANCE).addScalar("pointNicePrice", BigDecimalType.INSTANCE).addScalar("standardPrice", BigDecimalType.INSTANCE).addScalar("honourPrice", BigDecimalType.INSTANCE)
					.addScalar("multipleNumber", StringType.INSTANCE).addScalar("repertory", LongType.INSTANCE).addScalar("salesNum", LongType.INSTANCE).setResultTransformer(Transformers.aliasToBean(ProductInfoVo.class))
					.setProperties(parMap).setFirstResult((pageNo - 1) * pageSize).setMaxResults(pageSize).list();
		    //查出数量
		    String sql0 = sql.toString().replace("SELECT p.ID_ AS id,p.NAME_ AS name,p.PRICE_ AS price,p.IMG_URL_ AS imgUrl,p.IS_CAN_USER_COU_ as isCanUserCou,p.POINT_NICE_PRICE_ as pointNicePrice,p.STANDARD_PRICE_ as standardPrice,p.HONOUR_PRICE_ as honourPrice,p.MULTIPLE_NUMBER_ as multipleNumber,p.REPERTORY_ as repertory,p.SALES_NUM_ as salesNum",
					"select count(1)");
		    BigInteger count = (BigInteger) getCurrentSession().createSQLQuery(sql0.toString()).setProperties(parMap).uniqueResult();

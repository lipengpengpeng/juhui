					 ...
					 <div class="cash-box left">
					  ...
					  <div class="cash-box left">
					     ...
					     <c:if test="${list.tradeType==1}">
					      ...
					      <c:if test="${list.tradeType==7}">
					        <i class="fuli-icon" style="background:none;background-size: cover;width: 5.5rem;margin: 1.5rem 2.5rem;">赠送代金券</i>
					     </c:if>
					       <c:if test="${list.tradeType==8}">
					        <i class="fuli-icon" style="background:none;background-size: cover;width: 5.5rem;margin: 1.5rem 2.5rem;">充值商家联盟</i>
					     </c:if>
					     <c:if test="${list.tradeType==9}">
					        <i class="fuli-icon" style="background:none;background-size: cover;width: 5.5rem;margin: 1.5rem 2.5rem;">赠送商家联盟</i>
					     </c:if>
					     <c:if test="${list.tradeType==10}">
					        <i class="fuli-icon" style="background:none;background-size: cover;width: 5.5rem;margin: 1.5rem 2.5rem;">奖励转入</i>
					     </c:if>
					     <c:if test="${list.tradeType==11}">
					        <i class="fuli-icon" style="background:none;background-size: cover;width: 5.5rem;margin: 1.5rem 2.5rem;">订单退款</i>
					     </c:if>
						
					  </div>
					  <c:if test="${list.tradeType==0||list.tradeType==3||list.tradeType==6||list.tradeType==10||list.tradeType==11}">
					    <div class="cash-box left">
						  <div class="cash-text shouxin-text" style="line-height: 4.5rem;">+${list.memberTreasureTradeMoney}</div>
					    </div>
					  </c:if>
					  <c:if test="${list.tradeType==2||list.tradeType==4||list.tradeType==5||list.tradeType==7||list.tradeType==8||list.tradeType==9}">
					    <div class="cash-box left">

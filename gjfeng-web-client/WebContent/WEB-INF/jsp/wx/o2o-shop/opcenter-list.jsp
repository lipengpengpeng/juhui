...
<body>
    <div class="container">
        <div class="agentList-top clearfix">
            ...
            <c:if test="${not empty resultVo.result.mchInfo}">${resultVo.result.mchInfo}</c:if>
      		<c:if test="${empty resultVo.result.mchInfo}">0</c:if>
            个</div>
        </div>
        <ul class="agentList-ul">
        	<c:if test="${empty resultVo.result.mchTotal}">
        	<div class="data-state-box">
		       <img src="${imagePath}/wx/o2o-shop/data-null.png" class="data-state-img">
		       <p class="data-state-text">没有数据</p>
		   </div>
        	</c:if>
        	<c:if test="${not empty resultVo.result.mchTotal}">
        	<c:forEach items="${resultVo.result.mchTotal}" var="bean" >

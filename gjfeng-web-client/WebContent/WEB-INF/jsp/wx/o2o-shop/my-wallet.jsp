...
<body>
	<div class="container">
		...
		<div class="wallet-tab-box" id="wallet-1-2">
			<div class="wallet-m-top clearfix">
				<div class="wm-t-1 left">
				    <a href="${ctx}/wx/trade/getParticipate?type=0">
						<p class="wm-text">用户余额</p>
					    <div class="wm-money" id="memNum2">${resultVo.result.memberInfo.balanceMoney}</div>
					</a>
					
				</div>
				<div class="wm-t-2 left">
					<p class="wm-text">代金券</p>
					<div class="wm-money" id="memNum3">${resultVo.result.memberInfo.memberVoucherMoney}</div>
				</div>
				<div class="wm-t-3 left">
					<p class="wm-text">总消费额</p>
					<div class="wm-money" id="memNum4">${resultVo.result.memberInfo.cumulativeMoney}</div>
				</div>
			</div>
			...
			<div class="wallet-middle clearfix">
				<div class="wm-item left" >
				    <a href="${ctx}/wx/trade/getSalesWelfare">
					    <p class="wm-text">一类消费金额</p>
					    <div class="wm-money" id="memNum5">${resultVo.result.memberInfo.merchantFirstCumulativeMoney }</div>
					</a>
					
				</div>
				<div class="wm-item left">
				    <a href="${ctx}/wx/trade/getInterests?type=0">
					    <p class="wm-text">一类代领消费金额</p>
					   <div class="wm-money" id="memNum6">${resultVo.result.memberInfo.merchantFirstCousumptionMoney}</div>
					</a>
					
				</div>
				<div class="wm-item left">
					<p class="wm-text">一类分红权</p>
					<div class="wm-money" id="memNum7">${resultVo.result.memberInfo.merchantFirstDiviNum }</div>
				</div>
			</div>
			...
			<div class="wallet-middle clearfix">
				<div class="wm-item left">
					<p class="wm-text">二类消费金额</p>
					<div class="wm-money" >${resultVo.result.memberInfo.merchantSecondCumulativeMoney}</div>
				</div>
				<div class="wm-item left">				   
					<p class="wm-text">二类待领消费金额</p>
					<div class="wm-money" >${resultVo.result.memberInfo.merchantSecondCousumptionMoney}</div>
					<p class="wm-text"></p>
					<div class="wm-money" ></div>						  
				</div>
				<div class="wm-item left">
					<p class="wm-text">二类分红权</p>
					<div class="wm-money" >${resultVo.result.memberInfo.merchantSecondDiviNum}</div>
					
				</div>
				<div class="wm-item left">
					
					<p class="wm-text">三类消费金额</p>
					<div class="wm-money" >${resultVo.result.memberInfo.merchantThreeCumulativeMoney}</div>
				</div>
				<div class="wm-item left">
					
					<a href="">
					   <p class="wm-text">三类待领消费金额</p>
					   <div class="wm-money" >${resultVo.result.memberInfo.merchantThreeCousumptionMoney}</div>
					</a>					
				</div>
				<div class="wm-item left">
					
					<p class="wm-text">三类分红权</p>
					<div class="wm-money" >${resultVo.result.memberInfo.merchantThreeDiviNum}</div>
				</div>
			</div>
		</div>
		...
		<div class="wallet-menu clearfix">
			...
			<%-- <a href="${ctx}/wx/trade/toMerchantGiveToMember" class="wmenu-item left">
				<div class="wmenu-img">
					<img src="${ctx}/common/image/wx/o2o-shop/wallet-power.png">
				</div>
				<div class="wmenu-text">升级赠送</div>
			</a>  --%>
			...
			<c:if test="${sessionScope.gjfMemberInfo.type eq '1'&&sessionScope.gjfMemberInfo.merchantType!=0}">
			<a href="${ctx}/wx/trade/toVouchers" class="wmenu-item left">
				<div class="wmenu-img">
					<img src="${ctx}/common/image/wx/o2o-shop/wallet_redbag.png">
				</div>
				<div class="wmenu-text">赠送代金券</div>
			</a>
			</c:if>
        	...
        	$.ajax({
        		...
        		success:function(data){
        			if(data.result!=null&&data.result.confirmStatus=="0"&&data.result.benefitMoney>=5000){
        				layer.open({
        	                content: "尊敬的巨惠云商会员：感谢您对巨惠云商的支持，请您确认您此次在巨惠云商联盟商家消费的真实性，尊重并执行巨惠云商的消费“分红”规则，即按照平台每天营业额的一定比例，根据消费会员与商家的消费贡献值加权平均计算分配，用赠送的方式回馈给您，计算多少赠送多少，如对上述内容确认接受，请按“确认”键，系统自动将您此次消费所获得的“消费福利”转入您的福利账号！再次感谢您的支持！  声明：巨惠云商是“消费分红”平台，不是消费全返平台",     	                       
        	                btn: ['确认', '取消'],
